...
// ─── Runtime-discovered output fan-out (output_pattern, issue #227 item 5) ──

#[test]
fn cli_output_pattern_fanout_runs_and_resumes_without_rerunning_producer() {
    // The producer's outputs are enumerated at RUNTIME by a filesystem
    // scan; the deferred consumer instantiates once per discovered value.
    // Deleting the consumer outputs and re-running must NOT re-run the
    // producer — the persisted domain replays and the consumer
    // re-instantiates from it (checkpoint resume path).
    let dir = tempfile::tempdir().unwrap();
    let wf = dir.path().join("chunks.oxoflow");
    fs::write(
        &wf,
        r#"[workflow]
name = "chunks"
...
shell = "mkdir -p results/chunks && echo run >> producer_runs.txt && for i in 1 2 3; do echo part-$i > results/chunks/$i.txt; done"
...
"#,
    ...
    // Run 1: full fan-out, all three consumers execute.
    let out = oxo_flow_cmd()
        .args(["run", wf.to_str().unwrap()])
        .current_dir(dir.path())
        .output()
    ...
    for i in ["1", "2", "3"] {
        let content = fs::read_to_string(dir.path().join(format!("results/merged/{i}.txt")))
            .unwrap_or_else(|_| panic!("merged/{i}.txt missing"));
        assert_eq!(content, format!("part-{i}\n"));
    }

    // Delete the consumer outputs and resume: the producer must NOT re-run
    // (its outputs exist; the domain replays from the checkpoint) while the
    // consumers re-execute and regenerate their files.
    let checkpoint = dir.path().join(".oxo-flow/checkpoint.json");
    assert!(checkpoint.exists());
    for i in ["1", "2", "3"] {
        fs::remove_file(dir.path().join(format!("results/merged/{i}.txt"))).unwrap();
    }
    let out2 = oxo_flow_cmd()
        .args(["resume", checkpoint.to_str().unwrap()])
        .current_dir(dir.path())
    ...
    for i in ["1", "2", "3"] {
        let content = fs::read_to_string(dir.path().join(format!("results/merged/{i}.txt")))
            .unwrap_or_else(|_| panic!("merged/{i}.txt regenerated by resume"));
        assert_eq!(content, format!("part-{i}\n"));
    }
    // The producer must NOT have re-run: its counter file still records the
    // single execution from run 1 (checkpoint replay, not re-execution).
    let runs = fs::read_to_string(dir.path().join("producer_runs.txt")).unwrap();
    assert_eq!(
        runs.lines().count(),
        1,
        "resume must not re-run the output_pattern producer, got {} runs:\n{runs}",
        runs.lines().count()
    );
}

#[test]
fn cli_output_pattern_producer_failure_attributes_uninstantiated_consumers() {
    // Producer FAILURE leaves the deferred consumers uninstantiated — and
    // the failure output names them, loudly (never a silent zero-match).
    let dir = tempfile::tempdir().unwrap();
    let wf = dir.path().join("failsplit.oxoflow");
    fs::write(
        &wf,
        r#"[workflow]
name = "failsplit"
...
"#,
    ...
    let out = oxo_flow_cmd()
        ...
        .output()
        .unwrap();
    assert!(!out.status.success(), "producer failure must fail the run");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("not instantiated") && stderr.contains("split"),
        "failure must name the uninstantiated consumers, got: {stderr}"
    );
    assert!(
        stderr.contains("collect"),
        "failure must name the consumer rule 'collect', got: {stderr}"
    );
    assert!(
        !dir.path().join("results/merged/1.txt").exists(),
