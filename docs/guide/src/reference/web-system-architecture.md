...
The `handlers/` directory contains pre-v0.8 handler modules marked `#[deprecated(since = "0.8.0")]`. These are preserved for backward compatibility and will be removed no earlier than v0.9.3. New code should use `domains/*/` modules.
...
(Old `/workflows/*` endpoints marked `#[deprecated]` remain functional but will be removed in v0.9.3.)
