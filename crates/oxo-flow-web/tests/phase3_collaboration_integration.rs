...
fn app_personal() -> axum::Router { server::build_router("personal") }
fn app_team() -> axum::Router { server::build_router("team") }
fn app_hpc() -> axum::Router { server::build_router("hpc") }
...
async fn test_fork_endpoint_exists() {
    let req = Request::builder()
        ...
        .body(Body::from(json!({"user_id": "user-1"}).to_string())).unwrap();
    ...
    assert!(resp.status() == StatusCode::NOT_FOUND || resp.status() == StatusCode::SERVICE_UNAVAILABLE,
        "fork endpoint should respond (got {})", resp.status());
}
...
async fn test_share_endpoint_exists() {
    let req = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(json!({"visibility": "link", "expires_in_days": 30}).to_string())).unwrap();
    let resp = app_personal().oneshot(req).await.unwrap();
    assert!(resp.status() == StatusCode::NOT_FOUND || resp.status() == StatusCode::SERVICE_UNAVAILABLE,
        "share endpoint should respond (got {})", resp.status());
}
...
async fn test_import_endpoint_exists() {
    let req = Request::builder()
        ...
        .header("content-type", "application/json")
        .body(Body::from(json!({"url": "oxo+https://example.com/share/abc123"}).to_string())).unwrap();
    let resp = app_personal().oneshot(req).await.unwrap();
    // Accept 404 (not found) or 503 (DB unavailable) as both mean route exists
    assert!(resp.status() == StatusCode::NOT_FOUND || resp.status() == StatusCode::SERVICE_UNAVAILABLE,
        "import endpoint should respond (got {})", resp.status());
}
...
async fn test_import_invalid_url_format() {
    let req = Request::builder()
        ...
        .uri("/api/pipelines/import")
        .header("content-type", "application/json")
        .body(Body::from(json!({"url": "not-a-valid-oxo-url"}).to_string())).unwrap();
    let resp = app_personal().oneshot(req).await.unwrap();
...
async fn test_hpc_mode_has_hpc_route() {
    let req = Request::builder().method("GET").uri("/api/hpc").body(Body::empty()).unwrap();
    let resp = app_hpc().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
}
...
async fn test_all_modes_have_pipeline_routes() {
    for (mode, app) in [("personal", app_personal()), ("team", app_team()), ("hpc", app_hpc())] {
        let req = Request::builder()
            .method("POST").uri("/api/pipelines/parse")
            .header("content-type", "application/json")
            .body(Body::from(json!({"toml_content": "[workflow]\nname = \"t\""}).to_string())).unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert!(resp.status().is_success(), "{mode} mode should have pipeline parse");
    }
}
...
async fn test_all_modes_have_auth_routes() {
    for (mode, app) in [("personal", app_personal()), ("team", app_team()), ("hpc", app_hpc())] {
        let req = Request::builder()
            ...
            .header("content-type", "application/json")
            .body(Body::from(json!({"username":"admin","password":"admin"}).to_string())).unwrap();
        let resp = app.oneshot(req).await.unwrap();
...
async fn test_all_modes_have_ai_routes() {
    for (mode, app) in [("personal", app_personal()), ("team", app_team()), ("hpc", app_hpc())] {
        let req = Request::builder().method("GET").uri("/api/ai/config").body(Body::empty()).unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert!(resp.status().is_success(), "{mode} mode should have AI config");
    }
}
...
async fn test_license_header_present() {
    ...
    assert!(license_header.is_some(), "X-OxoFlow-License header must be present");
    let value_bytes = license_header.unwrap().as_bytes();
    let has_academic = std::str::from_utf8(value_bytes)
        .map(|s| s.contains("Academic"))
        .unwrap_or(false);
    assert!(has_academic || !value_bytes.is_empty(), "License header must be non-empty");
}
...
async fn test_version_header_present() {
    ...
    assert!(version_header.is_some(), "X-OxoFlow-Version header must be present");
}
