    ...
    match cli.command {
        ...
        Commands::DryRun { workflow, target } => {
            ...
            if cli.verbose {
                ...
                eprintln!(
                    "  {} threads total, {} MB memory total",
                    total_threads, total_memory_mb
                );

                if total_threads > system_threads {
                    eprintln!(
                        "  ⚠️ Will oversubscribe {} threads",
                        total_threads - system_threads
                    );
                }
                if total_memory_mb > system_memory_mb {
                    eprintln!(
                        "  ⚠️ May exceed memory by {} MB",
