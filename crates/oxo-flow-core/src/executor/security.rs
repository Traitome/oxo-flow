...
/// Compiled patterns for detecting secrets and credentials in workflow content.
#[allow(clippy::declare_interior_mutable_const)]
static SECRET_PATTERNS: LazyLock<Vec<(Regex, &str, &str)>> = LazyLock::new(|| {
    vec![
        (
            Regex::new(r#"(?i)(api[_\-]?key|apikey|auth[_\-]?token|access[_\-]?token)\s*[:=]\s*['\x22]?\w{8,}['\x22]?"#).unwrap(),
            "API_KEY",
            "Hardcoded API key or access token",
        ),
        (
            Regex::new(r#"(?i)(password|passwd|pwd|secret)\s*[:=]\s*['\x22]\S+['\x22]"#).unwrap(),
            "PASSWORD",
            "Hardcoded password or secret",
        ),
        (
            Regex::new(r"sk-(?:ant|proj)-[A-Za-z0-9_-]{20,}").unwrap(),
            "ANTHROPIC_KEY",
        ...
        (
            ...
            "OPENAI_KEY",
            "OpenAI / DeepSeek API key (sk-...)",
        ),
        (
            Regex::new(r"(?i)(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9]{36,}").unwrap(),
            "GITHUB_TOKEN",
            "GitHub personal access token",
        ),
        (
            Regex::new(r"(?i)(?:AKIA|ASIA)[A-Z0-9]{16}").unwrap(),
            "AWS_KEY",
            "AWS access key ID",
        ),
        (
        ...
        (
            Regex::new(r#"(?i)(?:jdbc|mysql|postgresql|mongodb)://[^@\s]+:[^@\s]+@"#).unwrap(),
            "DB_CREDENTIALS",
...
/// Scan a string for hardcoded secrets and credentials.
pub fn scan_for_secrets(content: &str) -> Vec<SecretFinding> {
    let mut findings = Vec::new();
    for (re, category, description) in SECRET_PATTERNS.iter() {
        for m in re.find_iter(content) {
            let redacted = if m.len() > 12 {
                format!("{}...{}", &m.as_str()[..4], &m.as_str()[m.len() - 4..])
            } else {
                "***".to_string()
            };
            findings.push(SecretFinding {
                ...
                location: format!("offset {}", m.start()),
...
impl std::fmt::Display for SecretFinding {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "[{}] {} at {}: {}",
            self.category, self.description, self.location, self.redacted_value
        )
    }
}
...
mod secret_tests {
    ...
    fn detect_openai_key() {
        let findings =
            scan_for_secrets("export OPENAI_API_KEY=sk-1234567890abcdef1234567890abcdef");
        assert!(!findings.is_empty());
    }

    #[test]
    fn detect_password() {
        let findings = scan_for_secrets(r#"password = "hunter2""#);
        assert!(!findings.is_empty());
    }

    #[test]
    fn detect_github_token() {
        let findings = scan_for_secrets("GITHUB_TOKEN=ghp_1234567890abcdefghijklmnopqrstuvwxyz");
        assert!(!findings.is_empty());
    }
    ...
    fn no_false_positive_on_safe_strings() {
        let findings = scan_for_secrets("threads = 4\nmemory = \"8G\"\nshell = \"echo hello\"");
        assert!(findings.is_empty());
    }

    #[test]
    fn redact_secret_value() {
        let findings = scan_for_secrets("DEEPSEEK_API_KEY=sk-00000000000000000000000000000000");
        assert!(!findings.is_empty());
        assert!(findings[0].redacted_value.contains("..."));
    }

    #[test]
    fn detect_db_connection_string() {
        let findings = scan_for_secrets("postgresql://admin:secret123@localhost:5432/mydb");
        assert!(findings.iter().any(|f| f.category == "DB_CREDENTIALS"));
    }
}
